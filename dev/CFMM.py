...
class UniV2(CFMM):
    ...
    def getMarginalPriceAfterXTrade(self, deltax, numeraire):
        ...
        if numeraire == "y":
            return (
                self.gamma
                * self.TradingFunction()
                / (self.x + self.gamma * deltax) ** 2
            )
        elif numeraire == "x":
            return 1 / (
                self.gamma
...
class RMM01(CFMM):
    ...
    def swapXforY(self, deltax, numeraire):
        ...
        tau = self.T - self.timescale * self.env.now
        # Normalize delta x to 1 unit
        new_y_reserves = self.TradingFunction() + self.K * norm.cdf(
            norm.ppf(1 - (self.x + self.gamma * self.scaleDown(deltax)))
            - self.vol * np.sqrt(tau)
        )
    ...
    def swapYforX(self, deltay, numeraire):
        ...
        assert nonnegative(deltay)
        tau = self.T - self.timescale * self.env.now
        new_x_reserves = 1 - norm.cdf(
            norm.ppf(
                (
                    (self.y + self.gamma * self.scaleDown(deltay))
                    - self.TradingFunction()
                )
                / self.K
            )
    ...
    def getMarginalPriceAfterXTrade(self, deltax, numeraire):
        ...
        def g(delta):
            return (
                self.K
                * np.exp(norm.ppf(1 - self.x - delta) * self.vol * np.sqrt(tau))
                * np.exp(-0.5 * tau * self.vol**2)
            )
    ...
    def getMarginalPriceAfterYTrade(self, deltay, numeraire):
        ...
        def g(delta):
            return (
                (1 / self.K)
                * np.exp(
                    -norm.ppf((self.y + delta - self.TradingFunction()) / self.K)
                    * self.vol
                    * np.sqrt(tau)
                )
                * np.exp(-0.5 * tau * self.vol**2)
    ...
    def findArbitrageAmountYIn(self, m):
        ...
        def inverseG(ref_price):
            return (
                ...
                + self.K
                * norm.cdf(
                    -np.log(self.K * ref_price) / (self.vol * np.sqrt(tau))
                    - 0.5 * self.vol * np.sqrt(tau)
                )
            )
    ...
    def findArbitrageAmountXIn(self, m):
        ...
        def inverseG(ref_price):
            return (
                ...
                - 1
                + norm.cdf(
                    -np.log(ref_price / self.K) / (self.vol * np.sqrt(tau))
                    - 0.5 * self.vol * np.sqrt(tau)
                )
            )
