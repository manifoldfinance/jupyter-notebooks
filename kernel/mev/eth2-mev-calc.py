...
block_selection_frequency_flashbots = (
    58  # % of blocks seen by Flashbots-enabled miners contains Flashbots bundles
)
...
n_validators = [
    n
    for n in range(
        lower_bound_for_active_staked_eth // 32,
        int(upper_bound_for_active_staked_eth) // 32,
        1000,
    )
...
ideal_reward_with_mev = [
    (
        ...
        + (
            avg_mev_reward_per_block
            * average_blocks_proposed_per_year(n)
            * (block_selection_frequency_flashbots / 100)
        )
    )
...
ax.plot(
    [n for n in n_validators],
    [100 * r / 32 for r in ideal_reward],
    label="Ideal annual rewards without MEV",
)
ax.plot(
    [n for n in n_validators],
    [100 * r / 32 for r in ideal_reward_with_mev],
    label="Ideal annual rewards with MEV",
)
plt.axvline(
    x=validators_on_the_network_today,
    linestyle="dotted",
    label="Approximate amount of validators as of writing (~160k)",
)
...
ax.set_ylim(ymin=0)
ax.set_title(
    "Probability mass function (100,000 validators) — number of block proposal opportunities per year"
)
...
print(
    f"With 100,000 validators, the mean number of blocks proposed per validator per year is {avg:.2f}\n"
)
print(
    f"The unluckiest 1% of validators will have the opportunity to produce at most {int(lmu[0])} blocks in a year"
)
print(
    f"The median (average) validator will have the opportunity to produce {int(lmu[1])} blocks in a year"
)
print(
    f"The luckiest 1% of validators will have the opportunity to produce at least {int(lmu[2])} blocks in a year"
)
...
for i in range(len(full_reward)):
    r_att = attestation_reward[i]
    r_inc = inclusion_reward[i]
    l_reward_without_mev.append(
        r_att + r_inc * ((7 / 8) + (1 / 8) * l_bpo[i] / mean_bpo[i])
    )
    u_reward_without_mev.append(
        r_att + r_inc * ((7 / 8) + (1 / 8) * u_bpo[i] / mean_bpo[i])
    )
...
ax.plot(
    [n for n in n_validators],
    [100 * r / 32 for r in u_reward_without_mev],
    "g",
    label="Luckiest 1% of validators without MEV extraction",
)
ax.plot(
    [n for n in n_validators],
    [100 * r / 32 for r in l_reward_without_mev],
    "r",
    label="Unluckiest 1% of validators without MEV extraction",
)
ax.plot(
    [n for n in n_validators],
    [100 * r / 32 for r in full_reward],
    "y",
    label="Average validator without MEV extraction",
)
ax.set_xlabel("No of validators on the network")
ax.set_ylabel("Return per validator per year (%)")
plt.axvline(
...
ax.plot(
    [n for n in n_validators],
    [100 * r / 32 for r in u_reward],
    "g",
    label="Luckiest 1% of validators with MEV extraction",
)
ax.plot(
    [n for n in n_validators],
    [100 * r / 32 for r in l_reward],
    "r",
    label="Unluckiest 1% of validators with MEV extraction",
)
ax.plot(
    [n for n in n_validators],
    [100 * r / 32 for r in u_reward_without_mev],
...
ax.plot(
    [n for n in n_validators],
    [100 * r / 32 for r in full_reward_with_mev],
    "y",
    label="Average validator with MEV extraction",
)
ax.set_xlabel("No of validators on the network")
ax.set_ylabel("Return per validator per year (%)")
plt.axvline(
...
data = pd.read_csv(
    "./miner-REV.csv"
)  # relative path, dtype={'Miner Reward': np.int64})
...
for i in range(len(full_reward)):
    ...
    l_reward.append(r_att + r_inc * ((7 / 8) + (1 / 8) * l_bpo[i] / mean_bpo[i]))
    l_reward[i] = l_reward[i] + (
        l_bpo[i] * unlucky_rev_10 * (block_selection_frequency_flashbots / 100)
    )
    u_reward.append(r_att + r_inc * ((7 / 8) + (1 / 8) * u_bpo[i] / mean_bpo[i]))
    u_reward[i] = u_reward[i] + (
        u_bpo[i] * lucky_rev_10 * (block_selection_frequency_flashbots / 100)
    )
...
for i in range(len(full_reward)):
    ...
    l_reward.append(r_att + r_inc * ((7 / 8) + (1 / 8) * l_bpo[i] / mean_bpo[i]))
    l_reward[i] = l_reward[i] + (
        mean_bpo[i] * unlucky_rev_1 * (block_selection_frequency_flashbots / 100)
    )
    u_reward.append(r_att + r_inc * ((7 / 8) + (1 / 8) * u_bpo[i] / mean_bpo[i]))
    u_reward[i] = u_reward[i] + (
        mean_bpo[i] * lucky_rev_1 * (block_selection_frequency_flashbots / 100)
    )
...
for P in participation_rate:
    ...
    if P < 1:
        inclusion_reward = [
            (7 / 8) * r * P * math.log(P) / (P - 1) for r in base_reward
        ]
    else:
    ...
    total_reward = [
        accuracy_rewards[i] + inclusion_reward[i] + block_reward[i]
        for i in range(len(block_reward))
    ]

    total_reward_with_mev = [
        accuracy_rewards[i] + inclusion_reward[i] + block_reward[i] + mev_reward[i]
        for i in range(len(block_reward))
    ]
...
print(
    f"at P = {participation_rate[1]:.2f}, rewards without MEV fall by {100 * (1 - r_100000_without_mev[1] / r_100000_without_mev[0]):.2f}%, opposed to {100 * (1 - r_100000_with_mev[1] / r_100000_with_mev[0]):.2f}% with MEV"
)
print(
    f"at P = {participation_rate[2]:.2f}, rewards without MEV fall by {100 * (1 - r_100000_without_mev[2] / r_100000_without_mev[0]):.2f}%, opposed to {100 * (1 - r_100000_with_mev[2] / r_100000_with_mev[0]):.2f}% with MEV"
)
print(
    f"at P = {participation_rate[3]:.2f}, rewards without MEV fall by {100 * (1 - r_100000_without_mev[3] / r_100000_without_mev[0]):.2f}%, opposed to {100 * (1 - r_100000_with_mev[3] / r_100000_with_mev[0]):.2f}% with MEV"
)
print(
    f"at P = {participation_rate[4]:.2f}, rewards without MEV fall by {100 * (1 - r_100000_without_mev[4] / r_100000_without_mev[0]):.2f}%, opposed to {100 * (1 - r_100000_with_mev[4] / r_100000_with_mev[0]):.2f}% with MEV"
)
...
ax.set_title(
    "Expected annual net rewards against validator downtime\n"
