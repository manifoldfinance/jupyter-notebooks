...
if False:
    ...
    print("\n----- Swap Amount X In ----- \n")
    amount_in = 0.1
    expected_amount_out = initial_y - K * norm.cdf(
        norm.ppf(1 - (initial_x + gamma * amount_in)) - sigma * np.sqrt(maturity)
    )
    ...
    print("\n----- Swap Amount Y In ----- \n")
    amount_in = 100
    expected_amount_out = initial_x - (
        1
        - norm.cdf(
            norm.ppf((initial_y + gamma * amount_in - 0) / K)
    ...
    print(
        "Expected with 0 fees: ",
        K
        * norm.pdf(norm.ppf(1 - initial_x) - sigma * np.sqrt(maturity))
        * quantilePrime(1 - initial_x),
    )
    ...
    print(
        "No-arbitrage bounds: ",
        rmm01Pool.getMarginalPriceAfterXTrade(0, "y"),
        " | ",
        rmm01Pool.getMarginalPriceAfterYTrade(0, "y"),
        "\n",
    )
    ...
    print(
        "No-arbitrage bounds after trade: ",
        rmm01Pool.getMarginalPriceAfterXTrade(0, "y"),
        " | ",
        rmm01Pool.getMarginalPriceAfterYTrade(0, "y"),
    )
...
if False:
    ...
    rmm01Pool = RMM01(
        initial_x, initial_y, fee, K, sigma, maturity, env, timescale, n_shares
    )
    ...
    expected_amount_out = initial_y - K * norm.cdf(
        norm.ppf(1 - (initial_x + gamma * rmm01Pool.scaleDown(amount_in)))
        - sigma * np.sqrt(maturity)
    )
    ...
    expected_amount_out = initial_x - (
        1
        - norm.cdf(
            norm.ppf((initial_y + gamma * rmm01Pool.scaleDown(amount_in) - 0) / K)
            + sigma * np.sqrt(maturity)
        )
    )
    expected_amount_out = rmm01Pool.scaleUp(expected_amount_out)
...
if False:
    ...
    fee = 0
    initial_x_rmm = 0.5
    initial_y_rmm = K * norm.cdf(
        norm.ppf(1 - initial_x_rmm) - sigma * np.sqrt(maturity)
    )
    timescale = 1
    # print("Initial x: ", initial_x)
    # print("Initial y: ", initial_y)
    ...
    rmm01Pool = RMM01(
        initial_x_rmm, initial_y_rmm, fee, K, sigma, maturity, env, timescale, n_shares
    )
    ...
    print(
        univ2pool.getMarginalPriceAfterXTrade(0, "y"),
        ", ",
        univ2pool.getMarginalPriceAfterYTrade(0, "y"),
        "\n",
    )
...
if True:
    ...
    test_pool_1 = PoolSimulation(
        env, [randomSwapAgent(env, 1000, 1000000)], UniV2(500, 500000, 0)
    )
